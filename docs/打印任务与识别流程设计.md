...
不建议在 `order_print_task` 冗余 `print_scope`、源文件、sheet 名、开发编号、箱号范围等字段：

- 打印层级可以通过 `order_detail_id` 是否为空判断。
- 源文件和订单/装箱单 sheet 名可以通过 `order_record` + `PrintType` 得到。
- 开发编号、箱号范围属于明细展示信息，需要时从 `order_record_detail` 或 `order_packing_detail` 查询。
...
flowchart TD
    ...
    E --> G["print_type=1, order_detail_id为空"]
    F --> H["print_type=2, order_detail_id为空"]
```
